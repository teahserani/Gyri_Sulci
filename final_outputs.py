...
output_data = {
    ...
    "Whole Brain Absolute Difference": abs_diff,
    "Whole Brain Gyri Sulci Ratio": ratio
}
...
for idx, roi in enumerate(roi_sulci_mu.keys()):
    ...
    output_data[f"Left{roi} - Gyri Stiffness"] = mean_gyri
    output_data[f"Left {roi} - Sulci Stiffness"] = mean_sulci
    output_data[f"Left {roi} - Gyri Sulci Percent Difference"] = percent_diff
    output_data[f"Left {roi} - Gyri Sulci Absolute Difference"] = abs_diff
    output_data[f"Left {roi} - Gyri Sulci Ratio"] = ratio


roi_sulci_mu = roi_rh_sulci_mu
roi_gyri_mu = roi_rh_gyri_mu
...
for idx, roi in enumerate(roi_sulci_mu.keys()):
    ...
    output_data[f"Right {roi} - Gyri Stiffness"] = mean_gyri
    output_data[f"Right {roi} - Sulci Stiffness"] = mean_sulci
    output_data[f"Right {roi} - Gyri Sulci Percent Difference"] = percent_diff
    output_data[f"Rightt {roi} - Gyri Sulci Absolute Difference"] = abs_diff
    output_data[f"Right {roi} - Gyri Sulci Ratio"] = ratio



in_out_cortex = f'{path}Ctx_Inner_Outer'
...
# Apply CSF mask to create corrected mu
mu = mu_with_csf * csf_mask
...
cortex_abs_diff = abs(avg_outer_cortex - avg_inner_cortex)

output_data["Outer Cortex Stiffness"] = float(avg_outer_cortex)
output_data["Inner Cortex Stiffness"] = float(avg_inner_cortex)
output_data["Inner Outer Cortex Percent Difference"] = float(cortex_diff)
output_data["Inner Outer Cortex Absolute Difference"] = float(cortex_abs_diff)
